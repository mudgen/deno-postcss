// Need source map support in deno before I can port this.
// More info here: https://github.com/denoland/deno/issues/4549
    ...
    if (prev) {
      if (typeof prev === "string") {
        ...
        if (prevPath && fs.existsSync && fs.existsSync(prevPath)) {
          ...
          return fs.readFileStrSync(prevPath, { encoding: "utf-8" }).toString()
            .trim();
        } else {
          throw new Error(
      ...
      if (fs.existsSync && fs.existsSync(map)) {
        return fs.readFileStrSync(map, { encoding: "utf-8" }).toString().trim();
      } else {
        return false;
      }
    }
