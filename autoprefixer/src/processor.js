import parser from "../../postcss-value-parser/src/parse.js";
import Value from "./value.js";
import gridUtils from "./hacks/grid-utils.js";
const insertAreas = gridUtils.insertAreas;
  ...
  add(css, result) {
    ...
    function insideGrid(decl) {
      return decl.parent.nodes.some((node) => {
        ...
        let displayGrid = node.prop === "display" &&
          /(inline-)?grid/.test(node.value);
      ...
      if (prop === "grid-row-span") {
        ...
        if (gridPrefixes) {
          if (/^(align|justify|place)-items$/.test(prop) && insideGrid(decl)) {
            ...
            /^(align|justify|place)-content$/.test(prop) && insideGrid(decl)
          ) {
            result.warn(
            ...
            let hasCols = parent.some((i) =>
              i.prop === "grid-template-columns"
            );
