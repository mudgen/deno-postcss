...
import list from "../../../postcss/src/list.js";
import util from "../utils.js";

const escapeRegexp = util.escapeRegexp;
const splitSelector = util.splitSelector;

const uniq = util.uniq;
...
function parseGridTemplatesData(css) {
  ...
  css.walkDecls(/grid-template(-areas)?$/, (d) => {
    ...
    let index = parsed.reduce((acc, { allAreas }, idx) => {
      let hasAreas = allAreas && areaNames.some((area) =>
...
function insertAreas(css, isDisabled) {
  ...
  css.walkDecls("grid-area", (gridArea) => {
    ...
    let gridAreaRuleIndex = gridAreaMedia
...
function autoplaceGridItems(decl, result, gap, autoflowValue = "row") {
  ...
  items.reverse().forEach((item, index) => {
    ...
    let nodeSelector = parent.selectors.map((sel) =>
      sel + ` > *:nth-child(${keys.length - index})`
    ).join(", ");
